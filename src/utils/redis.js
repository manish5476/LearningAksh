...
const cacheMiddleware = (duration = 300) => {
    return async (req, res, next) => {
        ...
        const userIdentifier = req.user ? `user:${req.user.id}` : 'public';
        const key = `cache:${userIdentifier}:${baseUrl}`;
...
/**
 * Clear Cache Middleware for Express
 * Automatically clears cache keys matching a pattern when a mutation is successful.
 */
const clearCacheMiddleware = (pattern) => {
    return async (req, res, next) => {
        // Wait for the response to finish sending back to the user
        res.on('finish', async () => {
            // Only blow away the cache if the database successfully updated (Status 200-299)
            if (res.statusCode >= 200 && res.statusCode < 300) {
                try {
                    const clearedCount = await safeCache.clear(pattern);
                    if (clearedCount > 0) {
                        console.log(`🧹 Cache cleared [${clearedCount} keys] for pattern: ${pattern}`);
                    }
                } catch (err) {
                    console.error('Clear Cache Middleware Error:', err.message);
                }
            }
        });
        
        // Pass control to your controller immediately
        next();
    };
};
...
module.exports = {
    ...
    // Middleware exactly mapped to your route imports
    cache: cacheMiddleware,
    clearCache: clearCacheMiddleware,
...
//         // MODIFIED: Create a more specific cache key
//         const baseUrl = req.originalUrl || req.url;
//         const userIdentifier = req.user ? `user:${req.user.id}` : 'public';
//         const key = `cache:${userIdentifier}:${baseUrl}`;
