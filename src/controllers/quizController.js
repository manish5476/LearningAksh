...
exports.createQuiz = catchAsync(async (req, res, next) => {
  const quiz = await QuizService.createQuiz(req.user.id, req.user.role, req.body);
  res.status(201).json({ status: 'success', data: { quiz } });
});

exports.submitQuiz = catchAsync(async (req, res, next) => {
  const data = await QuizService.submitQuiz(req.user.id, req.user.role, req.params.quizId, req.body.answers);
  res.status(200).json({ status: 'success', data });
});

exports.addQuestions = catchAsync(async (req, res, next) => {
  const { questions } = req.body;
  const quiz = await QuizRepository.findById(req.params.quizId, ['course']);
  ...
  const questionsWithQuiz = questions.map(q => ({ ...q, quiz: quiz._id }));
  const createdQuestions = await QuizQuestion.insertMany(questionsWithQuiz);

  // Use Repository methods to update totals
  ...
  await QuizRepository.updateById(quiz._id, {
    totalQuestions,
    totalPoints: totalPoints[0]?.total || 0
  });

  res.status(201).json({ status: 'success', results: createdQuestions.length, data: { questions: createdQuestions } });
});

exports.getQuizWithQuestions = catchAsync(async (req, res, next) => {
  const quiz = await QuizRepository.findById(req.params.id, ['course', 'lesson']);
  if (!quiz) return next(new AppError('Quiz not found', 404));

  const questions = await QuizQuestion.find({ quiz: quiz._id }).sort('order').lean();

  if (req.query.mode === 'take') {
    questions.forEach(q => {
      if (q.type === 'multiple-choice') q.options.forEach(opt => opt.isCorrect = undefined);
      else q.correctAnswer = undefined;
    });
  }
...
exports.getAllQuizzes = catchAsync(async (req, res, next) => {
  const result = await QuizRepository.findMany(req.query);
