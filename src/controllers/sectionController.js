...
exports.createSection = catchAsync(async (req, res, next) => {
  const section = await SectionService.createSection(req.user.id, req.user.role, req.body);
  res.status(201).json({ status: 'success', data: { section } });
});

exports.updateSection = catchAsync(async (req, res, next) => {
  const section = await SectionRepository.updateById(req.params.id, req.body);
  res.status(200).json({ status: 'success', data: { section } });
});

exports.deleteSection = catchAsync(async (req, res, next) => {
  await SectionService.deleteSection(req.user.id, req.user.role, req.params.id);
  res.status(200).json({ status: 'success', data: null });
});

exports.reorderSections = catchAsync(async (req, res, next) => {
  await SectionService.reorderSections(req.user.id, req.user.role, req.params.courseId, req.body.sections);
...
exports.cloneSection = catchAsync(async (req, res, next) => {
  const section = await SectionService.cloneSection(req.user.id, req.user.role, req.params.id);
  res.status(201).json({ status: 'success', data: { section } });
});

exports.getAllSections = catchAsync(async (req, res, next) => {
  const result = await SectionRepository.findMany(req.query);
  res.status(200).json({ status: 'success', results: result.results, data: { sections: result.data } });
});

exports.getSection = catchAsync(async (req, res, next) => {
  const section = await SectionRepository.findById(req.params.id, { path: 'course', select: 'title' });
  res.status(200).json({ status: 'success', data: { section } });
});
