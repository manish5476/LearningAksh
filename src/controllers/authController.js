...
/**
 * Formats and sends the JWT via cookie and JSON response
 */
...
exports.signup = catchAsync(async (req, res, next) => {
  // 1) Manual validation check (Fail fast before hitting DB)
  ...
  // 3) STATIC ROLE LOGIC
  // Convert to lowercase to match the static enum in User model
  let assignedRole = req.body.role ? req.body.role.toLowerCase().trim() : 'student';

  // Security: Prevent unauthorized 'admin' registration
  if (assignedRole === 'admin') {
    return next(new AppError('Unauthorized: Registration for Admin role is restricted.', 403));
  }

  // 4) Create user
  // This will trigger the User schema validation (enum check)
  ...
  try {
    ...
    // Rollback: delete the user if profile creation fails to prevent "orphaned" users
    console.error("CRITICAL: Profile creation failed during signup:", err);
    await User.findByIdAndDelete(newUser._id);
    return next(new AppError('Account creation failed during profile setup. Please try again.', 500));
  }
...
exports.login = catchAsync(async (req, res, next) => {
  ...
  // Include password field which is hidden by default in schema
  ...
  if (!user.isActive || user.isDeleted) {
    return next(new AppError('Your account has been deactivated or deleted', 401));
  }
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    await user.save({ validateBeforeSave: false });
    return next(new AppError('Email delivery failed. Try again later.', 500));
  }
});
