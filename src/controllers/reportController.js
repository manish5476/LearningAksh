 ...
 * GET /api/v1/reports/course/:courseId?format=pdf
 */
exports.generateCourseReport = catchAsync(async (req, res, next) => {
  const { format = 'pdf' } = req.query;

  // 1. Get raw data from Service
  const reportData = await ReportingService.getCourseReportData(req.params.courseId);

  // 2. Export based on requested format
  if (format === 'excel') {
    return await reportExporter.generateExcel(reportData, 'course', res);
  }

  // Default to PDF
  await reportExporter.generatePDF(reportData, 'course', res);
});

/**
 * GENERATE INSTRUCTOR OVERVIEW REPORT
 * GET /api/v1/reports/instructor/:instructorId
 */
exports.generateInstructorReport = catchAsync(async (req, res, next) => {
  const { format = 'pdf' } = req.query;

  const reportData = await ReportingService.getInstructorReportData(req.params.instructorId);

  if (format === 'excel') {
    return await reportExporter.generateExcel(reportData, 'instructor', res);
  }

  await reportExporter.generatePDF(reportData, 'instructor', res);
});

/**
 * GENERATE PLATFORM PERFORMANCE REPORT (Admin only)
 * GET /api/v1/reports/platform?startDate=...&endDate=...
 */
exports.generatePlatformReport = catchAsync(async (req, res, next) => {
  const { format = 'pdf', startDate, endDate } = req.query;

  const reportData = await ReportingService.getPlatformReportData(startDate, endDate);

  if (format === 'excel') {
    return await reportExporter.generateExcel(reportData, 'platform', res);
  }

  await reportExporter.generatePDF(reportData, 'platform', res);
});

/**
 * CUSTOM DYNAMIC REPORTING
 * POST /api/v1/reports/custom
 */
exports.generateCustomReport = catchAsync(async (req, res, next) => {
  const { metrics, filters, groupBy, collection } = req.body;
  
  // Custom reports usually return JSON for dashboard charts
  const results = await ReportingService.getCustomAggregation(collection, metrics, filters, groupBy);
