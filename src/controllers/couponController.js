...
exports.validateCoupon = catchAsync(async (req, res, next) => {
  const { coupon, discountAmount, finalPrice } = await CouponService.validateAndCalculateDiscount(
    req.body.code, 
    req.body.courseId
  );
...
exports.applyCoupon = catchAsync(async (req, res, next) => {
  // Now correctly applies to the Order model!
  await CouponService.applyCouponToOrder(req.body.code, req.body.orderId, req.body.discountAmount);
  
  res.status(200).json({ status: 'success', message: 'Coupon applied successfully' });
});

exports.deactivateCoupon = catchAsync(async (req, res, next) => {
  const coupon = await CouponService.deactivateCoupon(req.params.id, req.user.id);
...
exports.getAllCoupons = catchAsync(async (req, res, next) => {
  const result = await CouponRepository.findMany(req.query);
  res.status(200).json({ status: 'success', results: result.results, data: { coupons: result.data } });
});
