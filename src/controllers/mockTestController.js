...
exports.startAttempt = catchAsync(async (req, res, next) => {
  const data = await MockTestService.startAttempt(req.user.id, req.params.mockTestId);
  res.status(201).json({ status: 'success', data });
});

exports.submitAttempt = catchAsync(async (req, res, next) => {
  const data = await MockTestService.submitAttempt(req.user.id, req.params.attemptId, req.body.answers);
  res.status(200).json({ status: 'success', data });
});

exports.getMyAttempts = catchAsync(async (req, res, next) => {
  const result = await MockTestAttemptRepository.findMany(
    req.query, 
    { student: req.user.id }, 
    { path: 'mockTest', select: 'title category level totalMarks' }
  );
  res.status(200).json({ status: 'success', results: result.results, data: { attempts: result.data } });
});

exports.getAttemptDetails = catchAsync(async (req, res, next) => {
  const attempt = await MockTestAttemptRepository.findById(req.params.id, ['mockTest', 'answers.questionId']);
  // ... authorization check remains ...
  res.status(200).json({ status: 'success', data: { attempt } });
});

// Admin CRUD using Repositories
...
exports.getAllMockTests = catchAsync(async (req, res, next) => {
  const result = await MockTestRepository.findMany(req.query, {}, ['category', 'instructor']);
