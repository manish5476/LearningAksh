...
exports.createLiveSession = catchAsync(async (req, res, next) => {
  const session = await LiveSessionService.createSession(req.user.id, req.user.role, req.body);
  res.status(201).json({ status: 'success', data: { session } });
});

exports.joinSession = catchAsync(async (req, res, next) => {
  const data = await LiveSessionService.joinSession(req.user.id, req.user.role, req.params.id);
  res.status(200).json({ status: 'success', data });
});

exports.startSession = catchAsync(async (req, res, next) => {
  const session = await LiveSessionService.startSession(req.user.id, req.params.id);
...
exports.leaveSession = catchAsync(async (req, res, next) => {
  ...
  res.status(200).json({ status: 'success', message: 'Left session successfully' });
});

exports.getUpcomingSessions = catchAsync(async (req, res, next) => {
  const sessions = await LiveSessionRepository.model.find({
    startTime: { $gt: new Date() },
    status: { $in: ['scheduled', 'live'] },
    isDeleted: { $ne: true }
  }).populate('course', 'title thumbnail').limit(10).lean();
...
exports.getAllSessions = catchAsync(async (req, res, next) => {
  const result = await LiveSessionRepository.findMany(req.query);
  res.status(200).json({ status: 'success', results: result.results, data: { sessions: result.data } });
});
