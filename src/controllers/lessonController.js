...
exports.createLesson = catchAsync(async (req, res, next) => {
  const lesson = await LessonService.createLesson(req.user.id, req.user.role, req.body);
  res.status(201).json({ status: 'success', data: { lesson } });
});

exports.updateLesson = catchAsync(async (req, res, next) => {
  const lesson = await LessonService.updateLesson(req.params.id, req.user.id, req.user.role, req.body);
...
exports.reorderLessons = catchAsync(async (req, res, next) => {
  await LessonService.reorderLessons(req.params.sectionId, req.user.id, req.user.role, req.body.lessons);
...
exports.getLessonWithDetails = catchAsync(async (req, res, next) => {
  const lesson = await LessonService.getLessonWithDetails(req.params.id, req.user.id, req.user.role);
  res.status(200).json({ status: 'success', data: { lesson } });
});
...
exports.markAsCompleted = catchAsync(async (req, res, next) => {
  const progressData = await LessonService.markAsCompleted(req.params.id, req.user.id);
...
exports.publishLesson = catchAsync(async (req, res, next) => {
  const lesson = await LessonService.togglePublishStatus(req.params.id, req.user.id, req.user.role, true);
  res.status(200).json({ status: 'success', data: { lesson } });
});

exports.unpublishLesson = catchAsync(async (req, res, next) => {
  const lesson = await LessonService.togglePublishStatus(req.params.id, req.user.id, req.user.role, false);
  res.status(200).json({ status: 'success', data: { lesson } });
});

exports.uploadVideo = catchAsync(async (req, res, next) => {
  // Pass to AWS S3 / Cloudinary Service here
...
exports.getAllLessons = catchAsync(async (req, res, next) => {
  const result = await LessonRepository.findMany(req.query);
  res.status(200).json({ status: 'success', results: result.results, data: { lessons: result.data } });
});
