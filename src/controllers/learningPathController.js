...
exports.createLearningPath = catchAsync(async (req, res, next) => {
  ...
  res.status(201).json({ status: 'success', data: { learningPath } });
});

exports.updateLearningPath = catchAsync(async (req, res, next) => {
  const learningPath = await LearningPathRepository.updateById(req.params.id, req.body);
  if (!learningPath) return next(new AppError('Learning path not found', 404));
  res.status(200).json({ status: 'success', data: { learningPath } });
});

exports.deleteLearningPath = catchAsync(async (req, res, next) => {
  await LearningPathRepository.deleteById(req.params.id);
...
exports.enrollInPath = catchAsync(async (req, res, next) => {
  await LearningPathService.enrollInPath(req.user.id, req.params.pathId);
  res.status(200).json({ status: 'success', message: 'Enrolled in learning path successfully' });
});

exports.getPathProgress = catchAsync(async (req, res, next) => {
  const pathData = await LearningPathService.getPathProgress(req.user.id, req.params.pathId);
  res.status(200).json({ status: 'success', data: pathData });
});

exports.getRecommendedPaths = catchAsync(async (req, res, next) => {
  const paths = await LearningPathService.getRecommendedPaths(req.user.id);
  res.status(200).json({ status: 'success', results: paths.length, data: { paths } });
});
...
exports.getAllLearningPaths = catchAsync(async (req, res, next) => {
  const result = await LearningPathRepository.findMany(req.query, {}, [
    { path: 'category', select: 'name' },
    { path: 'courses.course', select: 'title slug thumbnail' }
  ]);
  
  res.status(200).json({ status: 'success', results: result.results, data: { learningPaths: result.data } });
});

exports.getLearningPath = catchAsync(async (req, res, next) => {
  const learningPath = await LearningPathRepository.findById(req.params.id, [
    { path: 'category', select: 'name' },
    { path: 'courses.course', select: 'title slug thumbnail' }
  ]);
